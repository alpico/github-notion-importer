...
export async function run(): Promise<void> {
  ...
  if (!relatedPage) {
    throw new Error(
      'Please enter a page ID for where the issues related to this specific repo should be linked'
    )
  }
  ...
  while (true) {
    ...
    issues.forEach(
      async issue => await openIssue(notionClient, issue, relatedPage)
    )
...
async function paginatedIssues(
  repoName: string,
  repoOwner: string,
  next: string | undefined
  ...
  const ret = {
    issues: issues,
    next: paginator.pageInfo.hasPreviousPage
      ? paginator.pageInfo.startCursor
      : undefined
  }
  return ret
}
